...
On remarque que toutes les données sont numériques, on affiche les 10 premières lignes de nos données :

```{r}
head(data)
```

Puis on décide de formater les données pour donner un identifiant aux différentes lignes, dans notre cas, ce sera libellé :
...
Pour réaliser l'AFC, nous aurons besoin du package FactoMineR :

```{r}
library(FactoMineR)
```

Puis on range les résultats de l'AFC (valeurs propres, coordonnées, contribution) dans data.ca, dans notre cas, nous ne retenons que les 4 premiers axes et on souhaite que les graphiques ne soient pas générés lors de l'appel de la fonction :
